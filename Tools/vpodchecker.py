...
#   - SDDC Manager auto-rotate check reports credentials with rotation enabled
#     and whether rotation has been successfully disabled
#   - VCF Ops auth now tries both "local" and "localItem" authSources for
#     compatibility across VCF 9.0 and 9.1
#   - Firefox CA: vcf.lab Root Authority now WARN instead of FAIL (Vault root
#     CA is not present in all VCF versions)
#   - Firefox CA: "Broadcom, Inc CA" satisfied by "VCF Operations Fleet CA"
#     (they serve the same purpose, name varies by VCF version)
#   - Compatibility: get_lab_password() helper supports both lsf.get_password()
#     (current) and lsf.password (fy26hol)
# v2.3 - 2026-02-26:
...
def get_lab_password() -> str:
    """Get the lab password, compatible with both current and fy26hol lsfunctions."""
    if hasattr(lsf, 'get_password') and callable(lsf.get_password):
...
'''
    ...
    sections = [
        ...
        ('VCF Fleet Password Policies', report.fleet_password_policy_checks),
        ('Firefox Trusted Private CAs', report.firefox_ca_checks),
    ]
...
# CAs that are optional/version-dependent (WARN instead of FAIL when missing)
OPTIONAL_PRIVATE_CAS = {
    'vcf.lab Root Authority',
}

# CAs that are equivalent to each other (any one satisfies the requirement)
# Key = expected name, Value = set of alternative names that also satisfy it
EQUIVALENT_CAS = {
    'Broadcom, Inc CA': {'VCF Operations Fleet CA'},
}
...
def check_firefox_trusted_cas() -> List[CheckResult]:
    ...
    for expected_name in expected:
        ...
        # Check if an equivalent CA is present (e.g. VCF Operations Fleet CA
        # satisfies the Broadcom, Inc CA requirement)
        equivalents = EQUIVALENT_CAS.get(expected_name, set())
        if equivalents & found_names:
            satisfied_by = (equivalents & found_names).pop()
            results.append(CheckResult(
                name=f"Firefox CA: {expected_name}",
                status="PASS",
                message=f"Satisfied by equivalent: {satisfied_by}",
                details={'expected': expected_name, 'satisfied_by': satisfied_by,
                         'profile': profile_path}
            ))
            continue

        # Optional CAs get WARN instead of FAIL (version-dependent, e.g. the
        # Vault root CA is not present in every VCF version)
        if expected_name in OPTIONAL_PRIVATE_CAS:
            results.append(CheckResult(
                name=f"Firefox CA: {expected_name}",
                status="WARN",
                message="Not found (optional for this VCF version)",
...
def _get_ops_fqdn() -> Optional[str]:
    ...
    try:
        ...
        pass
    try:
        if lsf.config.has_section('VCF') and lsf.config.has_option('VCF', 'urls'):
            for entry in lsf.config.get('VCF', 'urls').split('\n'):
                url = entry.split(',')[0].strip()
                if 'ops-' in url:
                    from urllib.parse import urlparse
                    return urlparse(url).hostname
...
def _get_sddc_fqdn() -> Optional[str]:
    """Determine SDDC Manager FQDN from config."""
    if not lsf:
        return None
    try:
        if lsf.config.has_section('VCF'):
            host = lsf.config.get('VCF', 'sddcmgr', fallback=None)
            if host:
                return host
    except Exception:
        pass
    # Fallback: look in VMs list for sddcmanager entry
    try:
        if lsf.config.has_option('RESOURCES', 'VMs'):
            for line in lsf.config.get('RESOURCES', 'VMs').split('\n'):
                vm_name = line.split(':')[0].strip()
                if vm_name.startswith('sddcmanager'):
                    return f'{vm_name}.site-a.vcf.lab'
    except Exception:
        pass
    return None


def _acquire_ops_token(api_base: str, password: str) -> Optional[str]:
    """Authenticate to VCF Operations, trying both VCF 9.0 and 9.1 auth sources."""
    import requests
...
def _check_ops_fleet_policies(api_base: str, headers: dict) -> Optional[List[CheckResult]]:
    """
    Check VCF Ops internal password management policies (VCF 9.1+).

    Returns None if the API is not available (404), indicating the caller
    should fall back to the SDDC Manager auto-rotate check.
    """
    import requests
    results = []
    try:
    ...
    results.append(CheckResult(
        ...
        details={"policy_count": total_count}
    ))
    ...
    for policy in policies:
        ...
        try:
            ...
            exp_days = detail.get("expirationConstraint", {}).get(
                "passwordExpirationDays", "N/A")
        except Exception:
            exp_days = "N/A"
    ...
    for policy in policies:
        ...
        for resource in assigned_resources:
            ...
            assigned_date = (
                datetime.datetime.fromtimestamp(assigned_ts / 1000).strftime(
                    "%Y-%m-%d %H:%M") if assigned_ts else "Unknown")
            results.append(CheckResult(
                name=f"  Inventory: {res_name}",
                status="PASS",
                message=(f"Policy: {policy_name} | Type: {res_type} | "
                         f"Assigned: {assigned_date}"),
                details={"resourceName": res_name, "resourceType": res_type,
                         "policyName": policy_name, "assignedAt": assigned_date}
            ))

    return results


def _check_sddc_auto_rotate(sddc_fqdn: str, password: str) -> List[CheckResult]:
    """
    Check SDDC Manager credential auto-rotation policies (VCF 9.0+).

    Authenticates to the SDDC Manager API, enumerates all credentials that
    have an autoRotatePolicy configured, and reports whether auto-rotation
    has been disabled (PASS) or is still enabled (WARN).
    """
    import requests
    results = []
    ...
    # Authenticate
    token = None
    for username in ('administrator@vsphere.local', 'admin@local'):
        try:
            resp = requests.post(
    ...
    if not token:
        return [CheckResult(
            name="SDDC Auto-Rotate",
            status="WARN",
            message=f"Could not authenticate to SDDC Manager ({sddc_fqdn})"
        )]
    ...
    try:
        resp = requests.get(f'https://{sddc_fqdn}/v1/credentials',
                            headers=headers, verify=False, timeout=30)
        resp.raise_for_status()
        all_creds = resp.json().get('elements', [])
    except Exception as e:
        return [CheckResult(
            name="SDDC Auto-Rotate",
            status="WARN",
            message=f"Could not query credentials: {str(e)[:60]}"
        )]
    ...
    if not auto_rotate_creds:
        results.append(CheckResult(
            name="SDDC Auto-Rotate",
            status="PASS",
            message="No credentials with auto-rotate policies (rotation disabled)",
            details={'sddc_fqdn': sddc_fqdn, 'total_credentials': len(all_creds)}
        ))
        return results
    ...
    results.append(CheckResult(
        name="SDDC Auto-Rotate",
        status="WARN",
        message=(f"Found {len(auto_rotate_creds)} credential(s) with "
                 f"auto-rotate still enabled"),
        details={'sddc_fqdn': sddc_fqdn, 'count': len(auto_rotate_creds)}
    ))
    ...
    for cred in auto_rotate_creds:
        ...
        freq = policy.get('frequencyInDays', '?')
        next_sched = policy.get('nextSchedule', 'N/A')

        if next_sched and next_sched != 'N/A':
            try:
                dt = datetime.datetime.fromisoformat(
                    next_sched.replace('Z', '+00:00'))
                next_sched = dt.strftime('%Y-%m-%d')
            except Exception:
                pass

        results.append(CheckResult(
            name=f"  Auto-Rotate: {res_name}",
            status="WARN",
            message=(f"user={username} type={cred_type} "
                     f"rotate every {freq} days | next: {next_sched}"),
            details={
                ...
                'nextSchedule': next_sched
            }
        ))

    # Also check for recent failed rotation tasks
    try:
        tasks_resp = requests.get(f'https://{sddc_fqdn}/v1/credentials/tasks',
                                  headers=headers, verify=False, timeout=30)
        if tasks_resp.status_code == 200:
            tasks = tasks_resp.json().get('elements', [])
            failed_rotations = [t for t in tasks
                                if t.get('type') == 'ROTATE'
                                and t.get('status') == 'FAILED']
            if failed_rotations:
                results.append(CheckResult(
                    name="  SDDC Rotation Tasks",
                    status="WARN",
                    message=f"{len(failed_rotations)} failed rotation task(s) found",
                    details={'failed_count': len(failed_rotations)}
                ))
    except Exception:
        pass
...
def check_vcf_password_policies() -> List[CheckResult]:
    """
    Check VCF password policies and credential rotation settings.

    Strategy:
    1. Try VCF Operations internal password management API (VCF 9.1+)
    2. If that API returns 404 (VCF 9.0), fall back to checking SDDC Manager
       credential auto-rotate policies
    3. Always check SDDC Manager auto-rotate as an additional data point
    ...
    password = get_lab_password()
    ops_fqdn = _get_ops_fqdn()
    sddc_fqdn = _get_sddc_fqdn()

    # Attempt VCF Ops fleet password policies (9.1+ internal API)
    ops_results = None
    if ops_fqdn:
        api_base = f"https://{ops_fqdn}/suite-api"
        token = _acquire_ops_token(api_base, password)
        if token:
            headers = {
                "Authorization": f"OpsToken {token}",
                "Accept": "application/json",
            ...
            ops_results = _check_ops_fleet_policies(api_base, headers)

    if ops_results is not None:
        results.extend(ops_results)
    elif ops_fqdn:
        results.append(CheckResult(
            name="VCF Password Policies",
            status="INFO",
            message=(f"Fleet policy API not available on {ops_fqdn} "
                     f"(expected for VCF 9.0)")
        ))

    # SDDC Manager auto-rotate check (all VCF versions)
    if sddc_fqdn:
        sddc_results = _check_sddc_auto_rotate(sddc_fqdn, password)
        results.extend(sddc_results)
    else:
        results.append(CheckResult(
            name="SDDC Auto-Rotate",
            status="SKIPPED",
            message="SDDC Manager FQDN not found in config"
        ))
    ...
    if not results:
        results.append(CheckResult(
            name="VCF Password Policies",
            status="SKIPPED",
            message="No VCF Operations or SDDC Manager found in config"
        ))
