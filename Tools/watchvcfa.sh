...
POSTGRES="."
CCSK3SAPP="."
LABSTARTUPLOG="/home/holuser/hol/labstartup.log"
echo "$(date) -------------RUN START-------------" >> ${LABSTARTUPLOG}
echo "$(date +%T)-> VCFA Watcher started" >> ${LABSTARTUPLOG}
...
while [[ $REBOOTS -lt 3 || "$POSTGRES" != "2/2" ]]; do
   while ! $(sshpass -f /home/holuser/creds.txt ssh -q -o ConnectTimeout=5 "vmware-system-user@10.1.1.71" exit); do
        sleep 30
	echo "$(date +%T)-> Waiting for VCFA to come Online" >> ${LABSTARTUPLOG} 
   done
   echo "$(date +%T)-> VCFA online, reboot# $REBOOTS" >> ${LABSTARTUPLOG}
   sleep 60
   CNT=0
   while [[ "$POSTGRES" != "2/2" ]]; do 
	...
	((CNT++))
        echo "$(date +%T)-> PG Result: $POSTGRES - Attempt: $CNT" >> ${LABSTARTUPLOG}
	if [ $CNT -eq 30 ]; then
   ...
   while [[ "$CCSK3SAPP" != "2/2" ]]; do 
	...
	((CNT++))
        echo "$(date +%T)-> CCS Result: $CCSK3SAPP - Attempt: $CNT" >> ${LABSTARTUPLOG}
	if [ $CNT -eq 12 ]; then
    ...
    if [ "$CCSK3SAPP" == "2/2" ]; then
      ...
      echo "$(date +%T)-> Deleted CCS-K3S-APP for CPU usage bug" >> ${LABSTARTUPLOG}
      break
    fi
    if [ "$POSTGRES" == "2/2" ]; then
      echo "$(date +%T)-> Postgres is up, waiting for CCS-K3S-APP" >> ${LABSTARTUPLOG}
      break
    fi
    ((REBOOTS++))
done
