...
while [[ $REBOOTS -lt 3 || "$POSTGRES" != "2/2" ]]; do
   ...
   while [[ "$POSTGRES" != "2/2" ]]; do 
	...
	((CNT++))
        echo "$(date +%T)-> PG Running pods Result: $POSTGRES - Attempt: $CNT" 
	if [ $CNT -eq 5 ]; then
	  echo "$(date +%T)-> Rebooting after 5 minutes with Postgres only $POSTGRES"
   ...
   while [[ "$CCSK3SAPP" != "2/2" ]]; do 
	...
	((CNT++))
        echo "$(date +%T)-> CCS Running pods Result: $CCSK3SAPP - Attempt: $CNT" 
	if [ $CNT -eq 12 ]; then
