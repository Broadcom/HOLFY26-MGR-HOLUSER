...
def importCertificateToFleetManager(inFqdn,token, verify, alias, pemFile, keyFile):
    ...
    try:
        ...
        if response.status_code < 200 or response.status_code >= 300:
            ...
            vmid = jResponse.get('vmid')
            print(f"INFO: Certificate '{alias}' import complete (vmid={vmid}).")
            return vmid
...
def updateCertificateByVmid(inFqdn, token, verify, vmid, alias, pemFile, keyFile):

    if debug:
        print(f"In: updateCertificateByVmid")
    ...
    try:
        print(f"TASK: Updating Certificate {alias} ({vmid}) in Locker")
        response = requests.patch(url=url, data=payload, headers=headers, verify=verify )
        jResponse = response.json()

        if response.status_code < 200 or response.status_code >= 300:
            print(f"INFO: Response Code: {str(response.status_code)}")
            print(json.dumps(jResponse, indent=4))
            optResp = requests.options(url=url, headers=headers, verify=verify)
            print(f"INFO: Allowed methods on {url}: {optResp.headers.get('Allow')}")
            response.raise_for_status()
            return response.status_code
        else:
            print(f"INFO: Certificate '{alias}' update complete.")
            return vmid
