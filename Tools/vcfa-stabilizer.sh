...
# VCFA Complete Stabilization Script v2.10
# Version 2.10 - 2026-07-09
...
# components -> SDS NACK fix -> fsGroup perm fix (NEW Phase 3.6) -> stale pod cleanup (Phase 3.8)
# -> wait -> verify -> monitoring.
#
# VCF 9.0 back-port (v2.9):
#   This script was originally written for VCF 9.1 (envoy-gateway, kube-vip CP VIP=10.1.1.72).
#   VCF 9.0 differences handled by v2.9:
#   - API_SERVER default: 10.1.1.71:6443 (direct node) instead of 10.1.1.73; avoids dead-end
#     when the kube-vip CP VIP (10.1.1.70) is dropped after cold boot.
#   - kube-vip CP VIP: auto-detected from the manifest `address` field (10.1.1.70 in 9.0).
#   - vip_preserve_on_leadership_loss: VCF 9.0 kube-vip manifests lack this field; the patcher
#     now INSERTs it rather than only replacing existing values.
#   - Idempotency marker: switched from vip_leaseduration=60 (pre-set by VMware in 9.0) to
#     vip_preserve_on_leadership_loss (only added by this stabilizer).
#   - Gateway: VCF 9.0 uses Istio ingress (not envoy-gateway). The SDS NACK fix (Phase 3.5)
#     gracefully no-ops when no BackendTLSPolicy resources exist. The eg-mem-keeper watcher is
#     only installed when envoy-gateway is present.
#   - Pod cleanup: Phase 3.8 deletes Failed/Error/Evicted pods cluster-wide so the cluster
#     converges from a clean state (also available as standalone --clean-pods).
#
...
#   - fix-pgdata-perms init container in vcfapostgres postgresql CRD (via Zalando operator)
#   - fix-erlang-cookie-perms init container in rabbitmq-ha StatefulSet
# What's a runtime backstop (re-applied every run, harmless if already correct):
#   - eth0 VIP pinning for all kube-vip VIPs: CP VIP (auto-detected) + VMSP_GW + VCFA_GW
#   - ccs-k3s service-tls cert freshness check (v2.8)
#   - stale/failed pod cleanup: Failed, Error, Evicted pods deleted before stabilization wait (v2.9)
...
#
# v2.10 changelog (2026-07-09):
#  * NEW Phase 3.6: fix_fsgroup_permissions() — permanently patches two VCF 9.0 StatefulSets to
#    add chmod init containers that fix Kubernetes fsGroup permission corruption on every pod restart:
#      - vcfapostgres: Kubernetes fsGroup=103 changes /home/postgres/pgdata/pgroot/data from 0700
#        to 2770 (adds setgid + group rwx) → PostgreSQL FATAL: invalid permissions. Fix: init
#        container chmod 0700 via Zalando postgresql CRD initContainers field (v1.13.0 supported).
#        The Zalando operator reconciles the change into the StatefulSet automatically.
#      - rabbitmq-ha: fsGroup=200 changes .erlang.cookie from 0400 to 0440 (adds group read) →
#        Erlang "Cookie file must be accessible by owner only". Fix: init container chmod 0400
#        patched directly to the rabbitmq-ha StatefulSet (no operator managing it).
#    Both patches are idempotent (skipped if init container already present). The fix is permanent:
#    future pod restarts run the init containers automatically before the main process starts.
#    Also available as --fix-fsgroup standalone flag.
#
# v2.9 changelog (2026-07-09):
#  * VCF 9.0 back-port: API_SERVER default changed from 10.1.1.73:6443 to https://${VCFA_HOST}:6443
#    (direct node access, always reachable even when kube-vip drops the CP VIP).
#  * VCF 9.0 back-port: kube-vip CP VIP auto-detected from manifest `address` field in
#    fix_overload_recovery; overrides the configured VCFA_CP_VIP=10.1.1.72 default when the
#    manifest shows a different address (e.g. 10.1.1.70 on 9.0 labs).
#  * VCF 9.0 back-port: kube-vip manifest patcher now INSERTs vip_preserve_on_leadership_loss
#    after vip_retryperiod when the field is absent (was: only replaced existing values, leaving
#    9.0 manifests perpetually incomplete and causing repeated no-op write cycles).
#  * VCF 9.0 back-port: idempotency marker switched from vip_leaseduration=60 (already set by
#    VMware in VCF 9.0 kube-vip manifests) to vip_preserve_on_leadership_loss (only inserted
#    by this stabilizer). Prevents the script from exiting before doing any useful work on 9.0.
#  * NEW Phase 3.8: delete_stale_failed_pods() removes Failed, Error, OOMKilled, Evicted, and
#    CreateContainerConfigError pods cluster-wide (excluding kube-system static control-plane pods).
#    Called from main() between Phase 3.5 and Phase 4 (wait). Also available standalone via
#    --clean-pods.
#  * NEW --clean-pods flag: run only the stale pod cleanup without the full stabilization.
#  * VIP pinning deduplication: if VCFA_CP_VIP and VCFA_GW_VIP resolve to the same address
#    (VCF 9.0 where the CP and VCFA HTTP share 10.1.1.70), the pinning loop runs only once.
#
...
# VCF 9.0 back-port: use the node IP directly as the default API server.
# VCF 9.0 kube-vip CP VIP (10.1.1.70) may be dropped after cold boot;
# connecting directly to VCFA_HOST:6443 is always reachable.
# For VCF 9.1, set API_SERVER=https://10.1.1.72:6443 or export before running.
...
echo "=== 1/5: ensure CP VIP + gateway VIPs are on eth0 (non-deprecated) ==="
# VCF 9.0 back-port: auto-detect the kube-vip CP VIP from the static pod manifest so the
# correct IP is pinned regardless of whether this is a 9.0 or 9.1 lab.
# VCF 9.0: kube-vip address=10.1.1.70 (CP+SVC mode on same VIP); CP_VIP default=10.1.1.72 is wrong.
# VCF 9.1: kube-vip address=10.1.1.72; defaults remain correct.
KV_MANIFEST=/etc/kubernetes/manifests/kube-vip.yaml
DETECTED_CP_VIP=$(python3 -c "
import re
try:
    s=open('/etc/kubernetes/manifests/kube-vip.yaml').read()
    m=re.search(r'- name: address\s*\n\s+value: \"?([0-9.]+)\"?', s, re.M)
    print(m.group(1) if m else '')
except: pass
" 2>/dev/null || echo "")
if [[ -n "$DETECTED_CP_VIP" && "$DETECTED_CP_VIP" != "$CP_VIP" ]]; then
    echo "  kube-vip manifest: address=${DETECTED_CP_VIP} (overriding configured CP_VIP=${CP_VIP})"
    CP_VIP="$DETECTED_CP_VIP"
elif [[ -n "$DETECTED_CP_VIP" ]]; then
    echo "  kube-vip manifest: address=${DETECTED_CP_VIP} (matches configured CP_VIP)"
fi

# 'ip addr replace ... preferred_lft forever' is idempotent and refreshes a deprecated address.
# Build a deduplicated list: CP VIP + the two gateway VIPs.
declare -A _seen_vips=()
VIP_LIST=()
for v in "${CP_VIP}" "${VMSP_GW_VIP}" "${VCFA_GW_VIP}"; do
    [[ -z "$v" ]] && continue
    [[ -n "${_seen_vips[$v]:-}" ]] && continue
    _seen_vips[$v]=1
    VIP_LIST+=("$v")
done

for v in "${VIP_LIST[@]}"; do
...
def setv(text, name, val):
    ...
    if n == 0:
        # Field doesn't exist — insert after vip_retryperiod block (VCF 9.0 back-port)
        ins = '    - name: ' + name + '\n      value: "' + val + '"\n'
        m2 = re.search(r'(- name: vip_retryperiod\s*\n\s+value: "[^"]*"\n)', text, re.M)
        if m2:
            new = text[:m2.end()] + ins + text[m2.end():]
        else:
            new = text + ins
    return new
...
# Delete pods that are stuck in terminal states (Failed, Error, Evicted, OOMKilled) or have
# accumulated an excessive number of restarts, cluster-wide. Deployments and ReplicaSets will
# recreate deleted pods automatically; StatefulSet pods are deleted only when they are in a
# hard-failed state. This is called before the stabilization wait so the cluster has a clean
# slate to converge from.
#
# Exclusions: static control-plane pods in kube-system (etcd, kube-apiserver, kube-controller-manager,
# kube-scheduler, kube-vip) are never touched. Completed/Succeeded pods are also left alone.
#
# VCF 9.0 note: vsphere-cpi pods in kube-system may repeatedly Error due to cloud provider
# configuration — they are deleted so the DaemonSet can restart them, which often resolves
# transient credential/connectivity issues.
delete_stale_failed_pods() {
    log "Cleaning up stale/failed pods cluster-wide..."
...
# --- Static control-plane pod names (never delete these) ---
STATIC_PODS="etcd kube-apiserver kube-controller-manager kube-scheduler kube-vip"

is_static() {
    local name="\$1"
    for sp in \$STATIC_PODS; do
        [[ "\$name" == "\$sp"-* || "\$name" == "\$sp" ]] && return 0
    done
    return 1
}

echo "=== 1/3: delete Failed pods ==="
\$K get pods -A --field-selector=status.phase=Failed --no-headers 2>/dev/null \
  | awk '{print \$1, \$2}' \
  | while read -r ns pod; do
        is_static "\$pod" && { echo "  skip static: \$ns/\$pod"; continue; }
        echo "  deleting Failed pod: \$ns/\$pod"
        \$K delete pod "\$pod" -n "\$ns" --grace-period=0 --force 2>/dev/null || true
    done

echo "=== 2/3: delete Error/OOMKilled/CreateContainerConfigError/InvalidImageName pods ==="
\$K get pods -A --no-headers 2>/dev/null \
  | awk '\$4 ~ /^(Error|OOMKilled|CreateContainerConfigError|InvalidImageName|ErrImagePull|ImagePullBackOff)$/' \
  | awk '{print \$1, \$2, \$4}' \
  | while read -r ns pod reason; do
        is_static "\$pod" && { echo "  skip static: \$ns/\$pod (\$reason)"; continue; }
        echo "  deleting \$reason pod: \$ns/\$pod"
        \$K delete pod "\$pod" -n "\$ns" --grace-period=0 --force 2>/dev/null || true
    done

echo "=== 3/3: delete Evicted pods ==="
...
echo "=== done: stale pod cleanup complete ==="
REMOTE
)
    execute_remote "$clean_script" "Delete stale/failed/evicted pods (pre-stabilization cleanup)" true
}

# Fix Kubernetes fsGroup permission corruption on VCF Automation StatefulSets.
#
# When a pod restarts and Kubernetes mounts a PVC with fsGroup set, fsGroupChangePolicy=Always
# recursively changes file/directory ownership and permissions on the volume:
#   - Directories: setgid bit added + group rwx added (0700 → 2770)
#   - Regular files: group read added (0400 → 0440 for no-execute files)
#
# Two VCF Automation components break on pod restart because of this:
#   1. vcfapostgres (PostgreSQL/Patroni, fsGroup=103):
#      /home/postgres/pgdata/pgroot/data becomes 2770 → FATAL: invalid permissions
#   2. rabbitmq-ha (RabbitMQ/Erlang, fsGroup=200):
#      /var/lib/rabbitmq/.erlang.cookie becomes 0440 → Error: must be accessible by owner only
#
# Fix: add a chmod init container to each StatefulSet that runs AFTER fsGroup applies but
# BEFORE the main container starts. This executes on every pod restart automatically.
#
# vcfapostgres: patched via the Zalando postgresql CRD (initContainers supported in v1.13.0+).
#   The operator reconciles the CRD change into the StatefulSet, which triggers a pod restart;
#   the new pod starts with the init container that fixes the data dir before postgres launches.
# rabbitmq-ha: patched directly on the StatefulSet (no ownerReferences, no active operator).
#
fix_fsgroup_permissions() {
    log "Checking fsGroup permission fixes for vcfapostgres and rabbitmq-ha..."
...
echo "=== 1/2: vcfapostgres postgresql CRD init container ==="
IC_CHECK=\$(\$K get postgresql vcfapostgres -n prelude \
  -o jsonpath='{.spec.initContainers[*].name}' 2>/dev/null || echo "")
if echo "\$IC_CHECK" | grep -q "fix-pgdata-perms"; then
    echo "  fix-pgdata-perms already in postgresql CRD — skipping"
else
    SPILO_IMG=\$(\$K get statefulset vcfapostgres -n prelude \
      -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null \
      || echo "registry.vmsp-platform.svc.cluster.local:5000/images/zalando-spilo-16:3.3-p1")
    echo "  Patching postgresql CRD (image: \${SPILO_IMG})..."
    \$K patch postgresql vcfapostgres -n prelude --type=merge -p "{
      \"spec\": {
        \"initContainers\": [{
          \"name\": \"fix-pgdata-perms\",
          \"image\": \"\${SPILO_IMG}\",
          \"command\": [\"sh\",\"-c\",\"chmod 0700 /home/postgres/pgdata/pgroot/data 2>/dev/null && echo fixed-pgdata-perms || echo no-data-dir-yet\"],
          \"volumeMounts\": [{\"name\": \"pgdata\", \"mountPath\": \"/home/postgres/pgdata\"}],
          \"securityContext\": {\"runAsUser\": 0}
        }]
      }
    }" 2>&1 && echo "  postgresql CRD patched OK" || echo "  WARNING: CRD patch failed (operator may not support initContainers)"
    PATCHED=1
fi

echo "=== 2/2: rabbitmq-ha StatefulSet init container ==="
IC_CHECK=\$(\$K get statefulset rabbitmq-ha -n prelude \
  -o jsonpath='{.spec.template.spec.initContainers[*].name}' 2>/dev/null || echo "")
if echo "\$IC_CHECK" | grep -q "fix-erlang-cookie-perms"; then
    echo "  fix-erlang-cookie-perms already in rabbitmq-ha StatefulSet — skipping"
else
    RMQ_IMG=\$(\$K get statefulset rabbitmq-ha -n prelude \
      -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null \
      || echo "registry.vmsp-platform.svc.cluster.local:5000/images/prelude/rabbitmq:9.0.0.0.24701403")
    echo "  Patching rabbitmq-ha StatefulSet (image: \${RMQ_IMG})..."
    \$K patch statefulset rabbitmq-ha -n prelude --type=json -p "[{
      \"op\": \"add\",
      \"path\": \"/spec/template/spec/initContainers/-\",
      \"value\": {
        \"name\": \"fix-erlang-cookie-perms\",
        \"image\": \"\${RMQ_IMG}\",
        \"command\": [\"sh\",\"-c\",\"if [ -f /var/lib/rabbitmq/.erlang.cookie ]; then chmod 0400 /var/lib/rabbitmq/.erlang.cookie && echo fixed-cookie-perms; else echo cookie-not-found-yet; fi\"],
        \"volumeMounts\": [{\"name\": \"rabbit-pvc\", \"mountPath\": \"/var/lib/rabbitmq\"}],
        \"securityContext\": {\"runAsUser\": 100}
      }
    }]" 2>&1 && echo "  rabbitmq-ha StatefulSet patched OK" || echo "  WARNING: rabbitmq-ha patch failed"
    PATCHED=1
fi

if [[ "\$PATCHED" == "1" ]]; then
    echo "  Patches applied — waiting 15s for Zalando operator to reconcile StatefulSet..."
    sleep 15
fi
echo "=== done: fsGroup permission fixes verified ==="
REMOTE
)
    execute_remote "$fix_script" "Fix fsGroup permission corruption (vcfapostgres + rabbitmq-ha)" false
}

# Main execution function
main() {
    echo "======================================================================"
        echo "           VCFA Complete Stabilization Script v2.10"
    echo "======================================================================"
    echo "Comprehensive VCFA stability solution for nested environments (9.0 + 9.1)"
    echo "Default run: control-plane preflight + auth + core + SDS + pod cleanup + wait + verify"
    echo ""
    ...
    # VCF 9.0 back-port: vip_leaseduration=60 may already be set by VMware in VCF 9.0 kube-vip
    # manifests, so we use vip_preserve_on_leadership_loss as the canonical marker (the stabilizer
    # always inserts it; VCF 9.0 ships without it). For VCF 9.1 labs that also have
    # vcfa-eg-mem-keeper.sh, either marker is sufficient.
    local check_cmd="grep -q 'vip_preserve_on_leadership_loss' /etc/kubernetes/manifests/kube-vip.yaml 2>/dev/null || test -f /usr/local/bin/vcfa-eg-mem-keeper.sh"
    if vcfa_ssh_nosudo "$check_cmd" >/dev/null 2>&1; then
        echo "VCFA Stabilizer already applied (vip_preserve_on_leadership_loss present or eg-mem-keeper installed)..."
        exit 0
    fi
    ...
    echo ""
    info "=== PHASE 3.6: fsGroup Permission Fix (vcfapostgres + rabbitmq-ha) ==="
    fix_fsgroup_permissions

    echo ""
    info "=== PHASE 3.8: Stale/Failed Pod Cleanup ==="
    delete_stale_failed_pods
...
case "${1:-}" in
    --help|-h)
        ...
        echo "        3.5 SDS NACK auto-fix, 3.8 stale pod cleanup (v2.9), 4 wait, 5 verify, 6 monitoring."
        echo ""
        echo "VCF 9.0 back-port (v2.9):"
        echo "  Defaults updated for VCF 9.0: API_SERVER uses node IP, CP VIP auto-detected from"
        echo "  kube-vip manifest, idempotency marker updated (vip_preserve_on_leadership_loss)."
        echo "  For VCF 9.1: export API_SERVER=https://10.1.1.72:6443 before running."
        echo ""
        echo "The control-plane preflight (1.5) applies these durable fixes idempotently:"
        echo "  - pin gateway+CP VIPs on eth0 non-deprecated (kube-vip backstop; CP VIP auto-detected)"
        ...
        echo "  --preflight             Only Phase 1 + Phase 1.5 (control-plane preflight). Quick, idempotent."
        echo "                          Use after a fresh boot or before a known-busy run. ~2-3 min."
        echo "  --fix-fsgroup           v2.10: add chmod init containers to vcfapostgres (via Zalando CRD)"
        echo "                          and rabbitmq-ha StatefulSet to fix fsGroup permission corruption"
        echo "                          (postgres FATAL: invalid data dir perms; RabbitMQ .erlang.cookie error)."
        echo "  --clean-pods            v2.9: apply fsGroup fixes + delete Failed/Error/Evicted/OOMKilled pods"
        echo "                          cluster-wide (excludes kube-system static control-plane pods)."
        ...
        echo "                          (VCF 9.1 only — no BackendTLSPolicy resources in VCF 9.0 Istio-based gateway)"
        echo "  --fix-overload          Control-plane overload recovery (same internals as Phase 1.5, with"
