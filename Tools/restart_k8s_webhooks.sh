...
echo "==========================================" >> "${LOG_FILE}"
/usr/bin/sshpass -p "${nodePwd}" ssh "root@${nodeIP}" "kubectl -n svc-harbor-ns-domain-c10 scale sts --all --replicas=1" >> "${LOG_FILE}"
/usr/bin/sshpass -p "${nodePwd}" ssh "root@${nodeIP}" "kubectl -n svc-harbor-ns-domain-c10 scale deployment --all --replicas=1" >> "${LOG_FILE}"
