...
if [ "$vPod_SKU" = "HOL-BADSKU" ];then
   ...
   > /home/holuser/hol/vpodrouter/gitdone
...
prod=false
holdev=`vmtoolsd --cmd 'info-get guestinfo.ovfEnv' | grep -i HOL-Dev`
[ $? = 1 ] && prod=true
...
if [ "${labtype}" = "HOL" ];then
   /usr/bin/sshpass -p ${password} ssh -o ${sshoptions} holuser@router '> /tmp/vpodrouter/gitdone'
fi
