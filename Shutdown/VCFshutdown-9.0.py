...
def _shutdown_vm_gracefully(vm, lsf_ref=None):
    """
    Gracefully shut down a VM via pyVmomi. Tries ShutdownGuest first,
    polls powerState, falls back to PowerOffVM_Task on timeout or
    if VMware Tools is not running.

    Shim for lsf.shutdown_vm_gracefully which does not exist in VCF 9.0
    lsfunctions.py.
    """
    from pyVmomi import vim

    vm_name = getattr(vm, 'name', str(vm))
    _log = (lambda m: vcf_write(lsf_ref, m)) if lsf_ref else print

    power_state = vm.runtime.powerState
    if power_state != vim.VirtualMachinePowerState.poweredOn:
        _log(f'    {vm_name}: already {power_state}, skipping')
        return

    tools_status = getattr(vm.guest, 'toolsRunningStatus', '')
    if tools_status == 'guestToolsRunning':
        try:
            _log(f'    {vm_name}: sending ShutdownGuest...')
            vm.ShutdownGuest()
        except Exception as e:
            _log(f'    {vm_name}: ShutdownGuest failed ({e}), forcing power off')
            try:
        ...
        for _ in range(60):
            ...
            try:
                if vm.runtime.powerState != vim.VirtualMachinePowerState.poweredOn:
                    _log(f'    {vm_name}: powered off gracefully')
                    return
            except Exception:
                _log(f'    {vm_name}: VM object unavailable (likely powered off)')
                return
        _log(f'    {vm_name}: graceful shutdown timed out, forcing power off')
        try:
            vm.PowerOffVM_Task()
        except Exception:
            pass
    else:
        _log(f'    {vm_name}: VMware Tools not running, forcing power off')
        try:
            vm.PowerOffVM_Task()
        except Exception as e:
            _log(f'    {vm_name}: PowerOff failed: {e}')
...
def shutdown_vms_by_names(lsf, vm_names: list, dry_run: bool = False,
    ...
    for vm_name in vm_names:
        ...
        if use_regex:
            ...
            vms = lsf.get_vm(vm_name)
        if vms:
            for vm in vms:
                processed += 1
                _shutdown_vm_gracefully(vm, lsf)
                time.sleep(5)
        else:
...
def main(lsf=None, standalone=False, dry_run=False, phase=None):
    ...
    _fleet_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fleet-9.0.py')
    _fleet_spec = _ilu.spec_from_file_location('fleet', _fleet_path)
    fleet = _ilu.module_from_spec(_fleet_spec)
            ...
            if vcfcomponents:
                ...
                if not dry_run:
                    ...
                    if vsp_control_plane_ip:
                        ...
                        sudo_check = lsf.ssh('sudo -n true', f'{vsp_user}@{vsp_control_plane_ip}', password)
                        ...
                        def vsp_kubectl(kubectl_cmd):
                            ...
                            return lsf.ssh(ssh_cmd, f'{vsp_user}@{vsp_control_plane_ip}', password)
