...
def run_vcf_shutdown(lsf, dry_run: bool = False, phase=None) -> dict:
    ...
    """
    module = import_shutdown_module('VCFshutdown-9.0', lsf)
...
def _ping_once(hostname: str, timeout: int = 5) -> bool:
    """Single non-blocking ping. Returns True if host responds, False otherwise."""
    import subprocess
    try:
        ret = subprocess.run(
